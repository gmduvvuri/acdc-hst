...
def measure_gsag(corrtag, row_threshold=0.3):
    ...
    def prod(x0, y0, dx, dy):
        out = list(
            itertools.product(
                range(x0, x0 + dx + 1), range(y0, y0 + dy + 1)
            )
        )
        return out
    ...
    aa_lims = {
        "FUVA": (1060, 296, 15250, 734),  # xstart, ystart, xend, yend
        "FUVB": (809, 360, 15182, 785),
    ...
    for y in range(seg_aa_lims[1], seg_aa_lims[3] + 1):
        inds = np.where(
            (coords_y == y)
            & (coords_x > seg_aa_lims[0])
            & (coords_x < seg_aa_lims[2])
        )
...
def check_superdarks(binned_superdarks):
    ...
    for k in keys:
        if len(set(dark_vals[k])) != 1:
            print(
                f"WARNING!!!! Key {k} does not match for all superdarks"
            )
            bad = True
    if bad is True:
        for darkfile in binned_superdarks:
            print(darkfile)
        raise KeyError(
            "Not all input superdarks are binned identically"
        )
...
def bin_science(corrtag, b, fact=1, exclude_airglow=False):
    ...
    aperture_regions = get_aperture_region(
        cenwave=cenwave,
        aperture=aperture,
        segments=[segment],
        life_adj=[lp],
    )
    box = aperture_regions[segment][
        f"lp{lp}_{aperture.lower()}_{cenwave}"
    ]
        ...
        for x in range(combined_dark.shape[1]):
            if (
                ...
                csum = csum + 2.0 * (
                    ...
                    + binned_sci[y, x]
                    * (
                        np.log(binned_sci[y, x])
                        - np.log(combined_dark[y, x])
                    )
                )
            elif y not in excluded_rows and combined_dark[y, x] > 0:
                csum += np.abs(combined_dark[y, x]) * 2.0
            elif y not in excluded_rows and combined_dark[y, x] <= 0:
                csum += np.inf
...
def check_existing(filename, overwrite=False):
    if os.path.exists(filename) and overwrite is False:
        print(
            f"WARNING: File {filename} already exists and overwrite is False, skipping..."
        )
        return True
    else:
        return False
...
def predict_dark(
    ...
    for darkfile in binned_superdarks:
        ...
        im = ax.imshow(
            dark, aspect="auto", origin="lower", interpolation="nearest"
        )
        fig.colorbar(im, label="Counts", pad=0.01)
        ax.set_title(
            f"{os.path.basename(darkfile)}\n{dark_segment} {dark_hv} superdark",
            size=25,
        )
    ...
    for item in corrtags:
        ...
        if segment is not None:
            if file_segment != segment.upper() and str(file_hv) != str(
                hv
            ):
        ...
        excluded_rows, apertures = get_psa_wca(
            segment, cenwave, lp, binning
        )
        rootname0 = fits.getval(item, "rootname")
        rootname = rootname0.lower()
        binned_sci, nevents = bin_science(
            item,
            binning,
            exclude_airglow=exclude_airglow,
        )
        #        vmin = np.mean(binned_sci) - 1*np.std(binned_sci)
        #        if vmin < 0:
        #            vmin = 0
        vmin = 0
        vmax = np.median(binned_sci) + np.median(binned_sci) * 2.0
        if vmax < 6:
            vmax = 6
        ...
        ax.axhline(
            apertures["PSA"][0], lw=2, color=COLORS[3], label="PSA"
        )
        ...
        ax.axhline(
            apertures["WCA"][-1] + 1, lw=2, ls="dashed", color=COLORS[3]
        )
        ax.legend(loc="upper right")
        fig.colorbar(im, label="Counts", pad=0.01)
        figname = os.path.join(
            outdir, f"{rootname}_{segment}_binned_sci.png"
        )
        ax.set_title(
            f"{rootname} {segment} Binned Science Image", size=25
        )
        ...
        superdark_coeffs = [
            equal / (exptime / sci_exp)
            for exptime in superdark_exptimes
        ]
        ...
        # x0 = [0.007, 0.005]
        x0 = [
            0.1 * np.mean(binned_sci) / np.mean(dark_im)
            for dark_im in dark_ims
        ]
        #        x0 = superdark_coeffs
        res = minimize(
            fun_opt,
            ...
            bounds=[(1.0e-10, None) for x in binned_superdarks],
            options={"maxiter": 500},
        )
        ...
        vmin = (
            np.median(combined_dark1) - np.median(combined_dark1) * 0.5
        )
        if vmin < 0:
            vmin = 0
        vmax = (
            np.median(combined_dark1) + np.median(combined_dark1) * 2.0
        )
        fig, ax = plt.subplots(1, 1, figsize=(20, 8))
        im = ax.imshow(
            combined_dark1,
        ...
        ax.set_ylabel("Y (binned)")
        figname = os.path.join(
            outdir, f"{rootname}_{segment}_combined_dark.png"
        )
        exists = check_existing(figname, overwrite)
        if not exists:
            fig.savefig(figname, bbox_inches="tight")
        ...
        for i in range(len(bkg_rows)):
            ...
            smoothy_max = np.max(smoothy) * 2.0
            ylim = ax.get_ylim()
        ...
        fig.suptitle(
            f"Predicted vs. actual dark for non-PSA/WCA rows\n{rootname} {segment}",
            size=25,
            y=0.95,
        )
        figname = os.path.join(
            outdir, f"{rootname}_{segment}_predicted_dark_bkgd.png"
        )
        exists = check_existing(figname, overwrite)
        if not exists:
        ...
        # fig.legend(handles, labels, bbox_to_anchor=(.9, .91), loc="lower right", fontsize=18)
        figname = os.path.join(
            outdir, f"{rootname}_{segment}_predicted_dark_sci.png"
        )
        fig.suptitle(
            f"Predicted vs. actual dark across PSA rows\n{rootname} {segment}",
            size=25,
            y=0.97,
        )
        exists = check_existing(figname, overwrite)
        ...
        outfile = os.path.join(
            outdir, f"{rootname}_{segment}_noise_complete.asdf"
        )
